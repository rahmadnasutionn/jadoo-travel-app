        ...
        <div className="block lg:flex">
          <div className="w-full lg:w-6/12 px-0 lg:pl-8 lg:pr-20">
            <div className="flex flex-col mb-18">
              ...
              <h3 className="text-gray-900 text-4xl lg:text-5xl font-serif leading-tight lg:leading-snug mb-10">What people say about Us.</h3>
              <ul className="flex gap-x-6">
                {items.map((item, i) => {
                  return <li key={item.id} className={['w-2 h-2 rounded-full ', i === realSlide ? 'bg-gray-800' : 'bg-gray-300'].join(' ')} onClick={() => i !== realSlide && swiper?.current?.slideTo(i)}></li>;
                })}
              </ul>
            </div>
          </div>
          <div className="w-full lg:w-6/12 flex items-center justify-center relative h-[320px] lg:h-auto">
            <div className="absolute -inset-4 lg:-inset-10 pt-10 lg:-top-20">
              <Swiper
                        ...
                        <div className="relative">
                          <div className="bg-white shadow-great rounded-xl p-6 relative z-20">
                            <span className="w-12 h-12 absolute top-0 left-0 overflow-hidden transform -translate-x-1/2 -translate-y-1/2 rounded-full flex-none mr-3">
                              <Image layout="responsive" className="object-cover" width="100%" height="100%" src={`/images/${item.image}`} alt="Image Person" />
                            </span>
